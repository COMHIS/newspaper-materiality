...
wtnppagedata <- nppagedata %>% group_by(ISSN,year,month=floor_date(date, "month"),week=floor_date(date, "week")) %>% summarise(PAANIMEKE=first(PAANIMEKE),wmodecols=Mode(wmodecols),wmfont=Mode(wmfont),type=Mode(type),words=mean(words),area=sum(area),chars=sum(chars)) %>% ungroup() %>% arrange(ISSN,week)
...
npissuedata <- nppagedata %>% inner_join(npdatesbetween,by=c("issueId")) %>% group_by(ISSN,year,date,issueId) %>% summarise(PAANIMEKE=first(PAANIMEKE),datesbetween=first(datesbetween),pages=n(),type=Mode(type),height=mean(height),width=mean(width),area=mean(area),pheight=mean(pheight),pwidth=mean(pwidth),parea=mean(parea),wmodecols=Mode(wmodecols),modecols=Mode(modecols),wmediancols=median(wmediancols),mediancols=median(mediancols),presoftware=Mode(presoftware),preversion=Mode(preversion),ocrsoftware=Mode(ocrsoftware),ocrversion=Mode(ocrversion),wmfont=Mode(wmfont),wproportion=mean(wproportion),cmfont=Mode(cmfont),cproportion=mean(cproportion),amfont=Mode(amfont),aproportion=mean(aproportion),words=mean(words),chars=mean(chars)) %>% ungroup() %>% mutate(month=floor_date(date, "month"),week=floor_date(date, "week")) %>% arrange(ISSN,date,issueId)

ytnpissuedata <- nppagedata %>% inner_join(npdatesbetween,by=c("issueId")) %>% inner_join(nppages,by=c("issueId")) %>% group_by(ISSN,year) %>% summarise(PAANIMEKE=first(PAANIMEKE),datesbetween=Mode(datesbetween),pages=first(pages),type=Mode(type),area=mean(area),wmodecols=Mode(wmodecols),wmfont=Mode(wmfont),words=mean(words),chars=mean(chars)) %>% ungroup() %>% arrange(ISSN,year)
mtnpissuedata <- nppagedata %>% inner_join(npdatesbetween,by=c("issueId")) %>% inner_join(nppages,by=c("issueId")) %>% group_by(ISSN,year,month=floor_date(date, "month")) %>% summarise(PAANIMEKE=first(PAANIMEKE),datesbetween=Mode(datesbetween),pages=first(pages),type=Mode(type),area=mean(area),wmodecols=Mode(wmodecols),wmfont=Mode(wmfont),words=mean(words),chars=mean(chars)) %>% ungroup() %>% arrange(ISSN,month)
wtnpissuedata <- nppagedata %>% inner_join(npdatesbetween,by=c("issueId")) %>% inner_join(nppages,by=c("issueId")) %>% group_by(ISSN,year,month=floor_date(date, "month"),week=floor_date(date, "week")) %>% summarise(PAANIMEKE=first(PAANIMEKE),datesbetween=Mode(datesbetween),pages=first(pages),type=Mode(type),area=mean(area),wmodecols=Mode(wmodecols),wmfont=Mode(wmfont),words=mean(words),chars=mean(chars)) %>% ungroup() %>% arrange(ISSN,week)
