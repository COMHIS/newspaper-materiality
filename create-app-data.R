...
ytnpissuedata <- nppagedata %>% inner_join(npweeklytext,by=c("ISSN","week")) %>% inner_join(npdatesbetween,by=c("issueId")) %>% inner_join(nppages,by=c("issueId")) %>% group_by(ISSN,year) %>% summarise(PAANIMEKE=first(PAANIMEKE),datesbetween=Mode(datesbetween),pages=Mode(pages),type=Mode(type),area=mean(area),parea=mean(parea),wmodecols=Mode(wmodecols),wmfont=Mode(wmfont),words=mean(words),chars=mean(chars),twords=mean(twords),tchars=mean(tchars)) %>% ungroup() %>% arrange(ISSN,year)
mtnpissuedata <- nppagedata %>% inner_join(npweeklytext,by=c("ISSN","week")) %>% inner_join(npdatesbetween,by=c("issueId")) %>% inner_join(nppages,by=c("issueId")) %>% group_by(ISSN,year,month=floor_date(date, "month")) %>% summarise(PAANIMEKE=first(PAANIMEKE),datesbetween=Mode(datesbetween),pages=Mode(pages),type=Mode(type),area=mean(area),parea=mean(parea),wmodecols=Mode(wmodecols),wmfont=Mode(wmfont),words=mean(words),chars=mean(chars),twords=mean(twords),tchars=mean(tchars)) %>% ungroup() %>% arrange(ISSN,month)
wtnpissuedata <- nppagedata %>% inner_join(npweeklytext,by=c("ISSN","week")) %>% inner_join(npdatesbetween,by=c("issueId")) %>% inner_join(nppages,by=c("issueId")) %>% group_by(ISSN,year,month=floor_date(date, "month"),week=floor_date(date, "week",week_start=1)) %>% summarise(PAANIMEKE=first(PAANIMEKE),datesbetween=Mode(datesbetween),pages=Mode(pages),type=Mode(type),area=mean(area),parea=mean(parea),wmodecols=Mode(wmodecols),wmfont=Mode(wmfont),words=mean(words),chars=mean(chars),twords=mean(twords),tchars=mean(tchars)) %>% ungroup() %>% arrange(ISSN,week)
