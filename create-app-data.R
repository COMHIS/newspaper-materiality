...
mtnppagedata <- nppagedata %>% group_by(ISSN,year,month=floor_date(date, "month")) %>% summarise(PAANIMEKE=first(PAANIMEKE),wmodecols=Mode(wmodecols),type=Mode(type),words=mean(words),area=sum(area),chars=sum(chars)) %>% arrange(ISSN,month)
...
nppages <- nppagedata %>%group_by(ISSN,issueId) %>% summarise(pages=n())
npissuedata <- nppagedata %>% inner_join(npdatesbetween,by=c("ISSN","date")) %>% group_by(ISSN,year,date,issueId) %>% summarise(PAANIMEKE=first(PAANIMEKE),datesbetween=first(datesbetween),pages=n(),type=Mode(type),height=mean(height),width=mean(width),area=mean(area),wmodecols=Mode(wmodecols),modecols=Mode(modecols),wmediancols=median(wmediancols),mediancols=median(mediancols),words=mean(words),chars=mean(chars)) %>% arrange(ISSN,date,issueId)
ytnpissuedata <- nppagedata %>% inner_join(npdatesbetween,by=c("ISSN","date")) %>% inner_join(nppages,by=c("ISSN","issueId")) %>% group_by(ISSN,year) %>% summarise(PAANIMEKE=first(PAANIMEKE),datesbetween=Mode(datesbetween),pages=first(pages),type=Mode(type),height=mean(height),width=mean(width),area=mean(area),wmodecols=Mode(wmodecols),modecols=Mode(modecols),wmediancols=median(wmediancols),mediancols=median(mediancols),words=mean(words),chars=mean(chars)) %>% arrange(ISSN,year)
mtnpissuedata <- nppagedata %>% inner_join(npdatesbetween,by=c("ISSN","date")) %>% inner_join(nppages,by=c("ISSN","issueId")) %>% group_by(ISSN,year,month=floor_date(date, "month")) %>% summarise(PAANIMEKE=first(PAANIMEKE),datesbetween=Mode(datesbetween),pages=first(pages),type=Mode(type),height=mean(height),width=mean(width),area=mean(area),wmodecols=Mode(wmodecols),modecols=Mode(modecols),wmediancols=median(wmediancols),mediancols=median(mediancols),words=mean(words),chars=mean(chars)) %>% arrange(ISSN,month)
rm(nppages,npdatesbetween)

newspapers <- newspapers %>% left_join(npissuedata %>% group_by(ISSN) %>% summarise(fyear=min(year),lyear=max(year)),by=c("ISSN"))

save(list = ls(all.names = TRUE),file="app.RData")