...
inppagedata <- nppagedata %>% group_by(ISSN,year,issueId) %>% summarise(PAANIMEKE=first(PAANIMEKE),wmodecols=Mode(wmodecols),type=Mode(type),words=mean(words),area=sum(area),chars=sum(chars)) %>% ungroup() %>% arrange(ISSN,year,issueId)
...
npissuedata <- nppagedata %>% inner_join(npdatesbetween,by=c("ISSN","date")) %>% group_by(ISSN,year,date,issueId) %>% summarise(PAANIMEKE=first(PAANIMEKE),datesbetween=first(datesbetween),pages=n(),type=Mode(type),height=mean(height),width=mean(width),area=mean(area),wmodecols=Mode(wmodecols),modecols=Mode(modecols),wmediancols=median(wmediancols),mediancols=median(mediancols),words=mean(words),chars=mean(chars)) %>% ungroup() %>% arrange(ISSN,date,issueId)
